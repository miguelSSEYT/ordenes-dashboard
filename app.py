   ...
   # Asegurar tipo numérico para formateo correcto
    ...
    with st.expander("COOIS - Órdenes COMPLETAS que NO se pueden producir"):
        ...
        df['Reason'] = df.apply(lambda row: (
            ...
            str(int(row['Open Quantity'])) + " are available. Shortage: " +
            str(int(row['Order Quantity (Item)'] - row['Open Quantity']))
        ), axis=1)
    ...
    with st.expander("Material Requerido para Cumplir Producción"):
        ...
        faltantes_total = pd.concat([
            ...
            zco41_eval[~zco41_eval['Can Produce_order']][['Custom Description', 'Cantidad Faltante']]
        ])
